        ...
        <div class="col-md-4">
            <div class="card">
                <div class="card-header  sahba-card-header">
                    <h5 class="card-title text-white">@lang('admin.roles.add-role-to-permissions')</h5>
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-12 mt-3">
                            <form id="createPermissionForm" action="{{ route('roles.create') }}" method="post"
                                ...
                                <div class="form-group">
                                    <label for="">
                                        {{ __('admin.roles.persian_name') }}
                                    </label>
                                    <input type="text" name="persian_name" class="form-control">
                                </div>
                                <div class="form-group mt-5 text-center">
                                    <button
                                        class="btn  btn-outline-dark ">@lang('admin.roles.add-roles')</button>
                                </div>
                            </form>
                        </div>
                            ...
                            <div class="table-responsive">
                                <table id="myForm" class="table table-striped text-center">
                                    ...
                                    <tr role="row" class="title-row">
                                        <th>@lang('admin.roles.row')</th>
                                        <th>@lang('admin.roles.id')</th>
                                        <th>@lang('admin.roles.name')</th>
                                        <th>@lang('admin.roles.persian_name')</th>
                                        <th>@lang('admin.roles.operation')</th>
                                    </tr>
                                    </thead>
                                    ...
                                    @if($roles->count() > 0 )
                                        @foreach($roles as $r)
                                            <tr role="row">
                                                ...
                                                <td><a href="">{{ 1000+$r->id }}</a></td>
                                                <td>{{ $r->name }}</td>
                                                <td>{{ $r->persian_name }}</td>
                                                <td>

                                                    <div class="">
                                                        <a href="{{ route('add.permission.role',$r->id) }}" title="دسترسی های نقش">
                                                            <i class=" fa-solid fa-shield-halved"></i>
                                                        </a>
                                                        <form method="POST" id="remove-color-{{$r->id}}"
                                                              action="{{route('roles.destroy',['role'=>$r->id])}}"
